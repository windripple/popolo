...
namespace Popolo.CircuitNetwork
{
    ...
    public class CircuitSolver
    {
        ...
        private void initialize()
        {
            ...
            {
                odes = new OrdinaryDifferentialEquations(OrdinaryDifferentialEquations.SolverType.RungeKutta4, (uint)dynamicIndices.Length);
            }
            if (0 < staticIndices.Length)
            {
                mRoot = new MultiRoot((uint)staticIndices.Length);
            }
        }
        ...
        public void Solve()
        {
            ...
            if (0 < dynamicIndices.Length)
            {
                ...
                double initTime = 0;
                odes.Solve(resFunction1, ref dynamicPotentials, initTime, timeStep, 0.01, timeStep, 0);
            }
            else if(0 < staticIndices.Length)
            {
                mRoot.Solve(resFunction2, ref staticPotentials, 1e-5, 1e-5, 50, out iterNum);
            }
        }
        ...
        private void resFunction1(double time, double[] variables, ref double[] variablesDT)
        {
            //節点にエネルギーを設定
        ...
        private void resFunction2(double[] variables, ref double[] errors)
        {
            //節点にエネルギーを設定
