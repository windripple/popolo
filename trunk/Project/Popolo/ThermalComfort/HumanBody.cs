...
namespace Popolo.ThermalComfort
{
    ...
    public class HumanBody
    {
        ...
        public static bool UseQRDecomposition
        {
            get;
            set;
        }
        ...
        private void initialize(double weight, double height, double age, bool isMale, double cardiacIndexAtRest, double fatPercentage)
        {
            ...
            tauVector = new Vector(varSum);
            perm = new Permutation(varSum);
        ...
        private void updateBodyTemperature()
        {
            ...
            foreach (Nodes bp in parts.Keys)
            {
                ...
                bmView = new MatrixView(bmMatrix, startPoint, startPoint, size, size);
                zmView = new VectorView(zmVector, startPoint, size);
            ...
            {
                tauVector.SetValue(0);
                LinearAlgebra.QRDecomposition(ref bmMatrix, ref tauVector);
                LinearAlgebra.QRSolve(bmMatrix, tauVector, ref zmVector);
            }
            //LU分解による解法
            else
            {
                int sig;
                perm.Initialize();
                LinearAlgebra.LUDecomposition(ref bmMatrix, ref perm, out sig);
            ...
            foreach (Nodes bp in parts.Keys)
            {
                ...
                if (!part.IsJOSModel) size += 2;
                if (part.GetHeatCapacity(BodyPart.Segments.SuperficialVein) != 0) size += 1;
                zmView = new VectorView(zmVector, bpDict[bp], size);
